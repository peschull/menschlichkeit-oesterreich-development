...
class AuthHandler {
    ...
    async handleLogin(event) {
        ...
        try {
            ...
            this.showAlert('Ein unerwarteter Fehler ist aufgetreten. Bitte versuchen Sie es erneut.', 'danger');
        } finally {
    ...
    async handleRegister(event) {
        ...
        const validationErrors = this.validateRegistrationForm({
            firstName, lastName, email, password, passwordConfirm, privacyConsent
        });
    ...
    validateRegistrationForm(data) {
        ...
        if (!this.validateEmail(data.email)) {
            errors.push({ field: 'registerEmail', message: 'Bitte geben Sie eine gültige E-Mail-Adresse ein.' });
        }

        if (!data.password || data.password.length < 8) {
            errors.push({ field: 'registerPassword', message: 'Das Passwort muss mindestens 8 Zeichen lang sein.' });
        }

        if (data.password !== data.passwordConfirm) {
            errors.push({ field: 'registerPasswordConfirm', message: 'Die Passwörter stimmen nicht überein.' });
        }

        if (!data.privacyConsent) {
            errors.push({ field: 'privacyConsent', message: 'Sie müssen der Datenschutzerklärung zustimmen.' });
        }
    ...
    handleForgotPassword() {
        ...
        this.showAlert('Die Funktion "Passwort zurücksetzen" ist noch nicht verfügbar. Bitte kontaktieren Sie uns per E-Mail.', 'info');
    }
}
