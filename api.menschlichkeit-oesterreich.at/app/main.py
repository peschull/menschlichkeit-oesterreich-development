...
if frontend_origins_raw:
    allow_origins = [
        entry.strip() for entry in frontend_origins_raw.split(",") if entry.strip()
    ]
...
def _serialize_contact(contact: Dict[str, Any]) -> ContactResponse:
    email_value = (
        contact.get("email") or contact.get("email_primary") or contact.get("emailId")
    )
...
async def _civicrm_contact_get(
    *, contact_id: Optional[int] = None, email: Optional[str] = None
) -> Dict[str, Any]:
...
async def _civicrm_membership_update(
    membership_id: int, payload: Dict[str, Any]
) -> Dict[str, Any]:
...
async def civicrm_api_call(entity: str, action: str, params: dict):
    ...
    payload = {
        "params": params,
        "_authx": {"api_key": CIVI_API_KEY, "key": CIVI_SITE_KEY},
    }
    ...
    if isinstance(data, dict) and data.get("is_error"):
        raise HTTPException(
            status_code=400, detail=data.get("error_message", "CiviCRM error")
        )
...
async def register(request: RegisterRequest) -> ApiResponse:
    ...
    return ApiResponse(
        success=True, data=response.model_dump(), message="Registration successful"
    )
...
async def refresh_token(req: RefreshRequest) -> ApiResponse:
    ...
    data = {
        "tokens": tokens.model_dump(),
        "contact": (
            _serialize_contact(contact_record).model_dump() if contact_record else None
        ),
    }
...
async def get_user_profile(
    ...
    return ApiResponse(
        success=True,
        data=_serialize_contact(contact_record).model_dump(),
...
async def update_user_profile(
    update: ContactUpdate, payload: Dict[str, Any] = Depends(verify_jwt_token)
) -> ApiResponse:
    email = payload.get("sub")
    if not email:
        raise HTTPException(status_code=401, detail="Invalid token subject")
    ...
    if not any(
        [update.email, update.first_name is not None, update.last_name is not None]
    ):
    ...
    return ApiResponse(
        success=True,
        data=_serialize_contact(updated_contact).model_dump(),
...
async def create_contact(
    contact: ContactCreate, _: Dict[str, Any] = Depends(verify_jwt_token)
) -> ApiResponse:
    params: Dict[str, Any] = {
    ...
    return ApiResponse(
        success=True,
        data=_serialize_contact(created).model_dump(),
...
async def get_contact(
    contact_id: int = Path(..., gt=0), _: Dict[str, Any] = Depends(verify_jwt_token)
) -> ApiResponse:
...
async def create_membership(
    membership: MembershipCreate, _: Dict[str, Any] = Depends(verify_jwt_token)
) -> ApiResponse:
...
async def search_contacts(
    email: Optional[str] = None, _: dict = Depends(verify_jwt_token)
):
    ...
    try:
        ...
        return ApiResponse(success=True, data=result, message="Search completed")
...
async def create_sepa_mandate(
    mandate: SepaMandateCreate, _: dict = Depends(verify_jwt_token)
):
