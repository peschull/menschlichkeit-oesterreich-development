...
class FactCheckSpeedrun {
    ...
    renderClaims() {
        return this.currentClaims.map(claim => `
    ...
    endGame() {
        ...
        const results = {
            ...
            timeUsed: totalTime,
            timeBonus,
            averageResponseTime: this.playerAnswers.length > 0 ? 
                this.playerAnswers.reduce((sum, a) => sum + a.timestamp, 0) / this.playerAnswers.length : 0
        };
...
class BridgePuzzle {
    ...
    updateStability() {
        ...
        const avgStrength = totalConnections > 0 ? 
            this.bridges.reduce((sum, b) => sum + b.strength, 0) / totalConnections : 0;
    ...
    checkSolution() {
        const results = {
            ...
            networkCoverage: (new Set(this.bridges.flatMap(b => [b.from, b.to])).size / this.valueNodes.length) * 100,
            avgBridgeStrength: this.bridges.length > 0 ? 
                this.bridges.reduce((sum, b) => sum + b.strength, 0) / this.bridges.length : 0
        };
...
class DebateDuel {
    ...
    calculateDebateQuality() {
        ...
        const avgStrength = this.playerArguments.reduce((sum, arg) => sum + arg.strength, 0) / this.playerArguments.length;
    ...
    showResults(results) {
        ...
        const victory = results.audienceSupport > 60 ? 'victory' : results.audienceSupport < 40 ? 'defeat' : 'draw';
        ...
        container.innerHTML = `
            <div class="game-results glass-panel ${victory}">
                ...
                <div class="final-verdict">
                    ${victory === 'victory' ? '🏆 Du hast das Publikum überzeugt!' :
                      victory === 'defeat' ? '😔 Die KI war überzeugender.' :
