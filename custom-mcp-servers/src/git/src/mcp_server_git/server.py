...
def git_branch(
    repo: git.Repo, branch_type: str, contains: str | None = None, not_contains: str | None = None
) -> str:
    match contains:
        case None:
            contains_sha = (None,)
...
async def serve(repository: Path | None) -> None:
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        match name:
            ...
            case GitTools.DIFF:
                diff = git_diff(
                    repo, arguments["target"], arguments.get("context_lines", DEFAULT_CONTEXT_LINES)
                )
